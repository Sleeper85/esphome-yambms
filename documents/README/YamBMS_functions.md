...
3. At first bulk touch: CCL drops to a small post-bulk floor (`auto_ccl_ct_post_bulk_amps`, default `2A`) so charging does not keep pushing past bulk; a non-zero floor is used because some inverters ignore `0A`.
...
- `Auto CCL CT`: Enables or disables the function.
